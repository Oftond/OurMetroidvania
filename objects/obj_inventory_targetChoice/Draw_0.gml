...
var _offset = sprite_get_width(_accept_icon) + 70;
if (page == 0 && choice_amulet != undefined)
{
	draw_sprite(_accept_icon, 0, obj_inventory.x + 20 + _offset, obj_inventory.bbox_bottom - 80);
	if (obj_player.inventory.amulets[selected_index].is_equipped)
		draw_text(obj_inventory.x + _offset + round(_offset / 2), obj_inventory.bbox_bottom - 85, "снять");
	else
		draw_text(obj_inventory.x + _offset + round(_offset / 2), obj_inventory.bbox_bottom - 85, "надеть");
}
